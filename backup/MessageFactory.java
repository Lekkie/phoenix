...
public class MessageFactory<T extends IsoMessage> {
	...
	throws ParseException, UnsupportedEncodingException {
		...
		if (useBinary) {
        	...
        	String str = new String(buf, isoHeaderLength, 4, encoding);
		...
		if (index == null) {
			...
			//log.error(String.format("ISO8583 MessageFactory has no parsing guide for message type %04x [%s]", type, new String(buf)));
			return null;
		}
					...
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						if (val != null) {
							...
							if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN) {
								pos++;
							} else if (val.getType() == IsoType.LLLLVAR || val.getType() == IsoType.LLLVAR || val.getType() == IsoType.LLLBIN) {
								pos += 2;
							}
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						int valLength = 0;
						if(IsoType.BINARY == val.getType())
						{
							valLength = val.getLength();							
						}
						else
						{
							valLength = val.toString().getBytes(fpi.getCharacterEncoding()).length;
						} 
						pos += valLength;
						if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN) {
							pos += 2;
