...
public class IsoValue<T> implements Cloneable {
	...
	public IsoValue(IsoType t, T value, CustomField<T> custom) {
		...
		type = t;
		this.value = value;
		if (type == IsoType.LLVAR || type == IsoType.LLLVAR || type == IsoType.LLLLVAR || type == IsoType.LLLLLVAR || type == IsoType.LLLLLLVAR) { // Added by Lekan Omotayo
			if (custom == null) {
			...
			else if (t == IsoType.LLLLLVAR && length > 99999) { // Added by Lekan Omotayo
				throw new IllegalArgumentException("LLLLLVAR can only hold values up to 99999 chars");
			}
			else if (t == IsoType.LLLLLLVAR && length > 999999) { // Added by Lekan Omotayo
				throw new IllegalArgumentException("LLLLLLVAR can only hold values up to 999999 chars");
			}
		} else if (type == IsoType.LLBIN || type == IsoType.LLLBIN || type == IsoType.LLLLBIN) { // Added by Lekan Omotayo
			if (custom == null) {
			...
			} else if (t == IsoType.LLLLBIN && length > 9999) { // Added by Lekan Omotayo
	...
	public IsoValue(IsoType t, T val, int len, CustomField<T> custom) {
		...
		} else if (t == IsoType.LLVAR || t == IsoType.LLLVAR || t == IsoType.LLLLVAR || t == IsoType.LLLLLVAR || t == IsoType.LLLLLLVAR) { // Added by Lekan Omotayo
			if (len == 0) {
		...
		} else if (t == IsoType.LLBIN || t == IsoType.LLLBIN || t == IsoType.LLLLBIN) { // Added by Lekan Omotayo
			if (len == 0) {
                if (custom == null) {
	...
	public String toString() {
		...
		if (type == IsoType.NUMERIC || type == IsoType.AMOUNT) {
			...
			return type.format(encoder == null ? value.toString() : encoder.encodeField(value), length);
		} else if (type == IsoType.LLVAR || type == IsoType.LLLVAR || type == IsoType.LLLLVAR || type == IsoType.LLLLLVAR || type == IsoType.LLLLLLVAR) {//Added by Lekan Omotayo
...
//		try
//		{
//			IsoValue<String> val = new IsoValue<String>(IsoType.LLLLLLVAR, "Testing");
//			val.writeLengthHeader(10000, outs, digits, binary, false);
//		}
//		catch(Exception ex)
//		{
//			ex.printStackTrace();
...
//	}
    ...
    protected void writeLengthHeader(final int l, final OutputStream outs, final int digits,
        ...
        if (binary) {
            if (digits == 6) {
            	outs.write((((l % 100000) / 100) << 4) | ((l % 100000)/100)); // 00000 to 99999
            }
			if (digits == 5) {
				outs.write((((l % 10000) / 100) << 4) | ((l % 10000)/100)); // 0000 to 9999 -- // Added by Lekan Omotayo
			}
            if (digits == 4) {
            	outs.write((((l % 1000) / 100) << 4) | ((l % 1000)/100)); // 000 to 999
            }
            if (digits == 3) {
			...
			else if (ldiff == 4) { // Added by Lekan Omotayo
				lhead = "0000" + lhead;
			}
            else if (ldiff == 5) {
                lhead = "00000" + lhead;
...
//                outs.write(((l%100000)/1000)+48);
//                outs.write(((l%100000)/100)+48);
            }
			if (digits == 5) { // Added by Lekan Omotayo
	...
	{
		if (type == IsoType.LLLLLLVAR || type == IsoType.LLLLLVAR || type == IsoType.LLLVAR || type == IsoType.LLVAR) { // Added by Lekan Omotayo
            writeLengthHeader(length, outs, (type == IsoType.LLLLLLVAR ? 6 : (type == IsoType.LLLLLVAR ? 5 : (type == IsoType.LLLLVAR ? 4 : (type == IsoType.LLLVAR ? 3 : 2)))), binary, forceStringEncoding); // Added by Lekan Omotayo
		} else if (type == IsoType.LLBIN || type == IsoType.LLLBIN || type == IsoType.LLLLBIN) { // Added by Lekan Omotayo
            writeLengthHeader(binary ? length : length*2, outs, (type == IsoType.LLLLBIN ? 4 : (type == IsoType.LLLBIN ? 3 : 2)), binary, forceStringEncoding); // Added by Lekan Omotayo
		} else if (binary) {
			...
			if (type == IsoType.NUMERIC) {
				...
				buf = new byte[6];
			} else if (type == IsoType.DATE12 || type == IsoType.DATE10 || type == IsoType.DATE4 || type == IsoType.DATE_EXP || type == IsoType.TIME) {// Added by Lekan Omotayo
		...
		if (binary && (type == IsoType.BINARY || type == IsoType.LLBIN || type == IsoType.LLLBIN || type == IsoType.LLLLBIN)) { // Added by Lekan Omotayo
			int missing = 0;
			if (value instanceof byte[]) {
